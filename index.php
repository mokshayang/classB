		...
		<div id="ms">
			<div id="lf" style="float:left;">
				<div id="menuput" class="dbor">
					...
					foreach($rows as $row){
						...
						$chk=$Menu->count(['parent'=>$row['id']]);
						if($chk>0){
							$opt=$Menu->all(['parent'=>$row['id']]);
							echo "<div class='mw'>";
							foreach($opt as $op){
								echo "<div class='mainmu2 cent' >";
								echo "<a href='{$op['href']}'>{$op['name']}</a>";
								echo "</div>";
							}	
							echo "</div>";
						}
			...
			<div class="di di ad" style="height:540px; width:23%; padding:0px; margin-left:22px; float:left; ">
				...
				<div style="width:89%; height:480px;" class="dbor">
					...
					<div class="cent ro" onclick="pp(1)"><img src="icon/up.jpg"></div>
					<?php
					$ims = $Image->all(['sh' => 1]);
					foreach ($ims as $idx => $im) {
					?>
						<div class="cent im" id="ssaa<?= $idx ?>">
							<img src="upload/<?= $im['img'] ?>" class="img" alt="pic">
						</div>
					<?php }	?>
					<div class="cent ro" onclick="pp(2)"><img src="icon/dn.jpg"></div>
					<script>
						var nowpage = 0,
							num = <?= count($ims) ?>;
